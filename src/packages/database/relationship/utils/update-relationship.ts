...
function updateHasOne({ record, value, opts, trx }: Params): Array<unknown> {
    ...
    if (value instanceof opts.model) {
      return [
        opts.model
          ...
          .where(`${opts.model.tableName}.${opts.foreignKey}`, recordPrimaryKey)
          .whereNot(
...
function updateHasMany({ record, value, opts, trx }: Params): Array<unknown> {
  const recordPrimaryKey = record.getPrimaryKey();
...
export default function updateRelationship(
  ...
  const value = dirtyRelationships.get(name) as
